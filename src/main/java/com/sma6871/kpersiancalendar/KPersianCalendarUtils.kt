...
object KPersianCalendarUtils {
	...
	fun julianToPersian(julianDate: Long): Long {
        val persianEpochInJulian =
            julianDate - persianToJulian(475L, 0, 1)
        val cyear =
            ceil(persianEpochInJulian.toDouble(), 1029983.0)
        val ycycle =
            if (cyear != 1029982L) Math.floor((2816.0 * cyear.toDouble() + 1031337.0) / 1028522.0)
                .toLong() else 2820L
        val year =
            474L + 2820L * Math.floor(persianEpochInJulian / 1029983.0).toLong() + ycycle
        val aux = 1L + julianDate - persianToJulian(year, 0, 1)
        val month =
            (if (aux > 186L) Math.ceil((aux - 6L).toDouble() / 30.0) - 1 else Math.ceil(
                aux.toDouble() / 31.0
            ) - 1).toInt()
        val day =
            (julianDate - (persianToJulian(year, month, 1) - 1L)).toInt()
